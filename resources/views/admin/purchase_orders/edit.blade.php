...
<div class="bg-white/80 dark:bg-slate-900/50 backdrop-blur p-6 rounded-2xl shadow ring-1 ring-slate-900/5 dark:ring-white/10">
    <h2 class="text-xl font-semibold text-slate-800 dark:text-slate-100 mb-6">Sửa phiếu nhập kho #{{ $purchaseOrder->code }}</h2>

    <form action="{{ route('admin.purchase_orders.update', $purchaseOrder->purchase_order_id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="grid gap-6 md:grid-cols-2">
            <div>
                ...
                <input type="date" name="order_date" id="order_date" value="{{ old('order_date', $purchaseOrder->order_date) }}" class="mt-1 block w-full rounded-xl border-slate-300 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100 focus:ring-brand-600 focus:border-brand-600" required>
                @error('order_date')
            ...
            <div>
                ...
                <select name="supplier_id" id="supplier_id" class="mt-1 block w-full rounded-xl border-slate-300 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100 focus:ring-brand-600 focus:border-brand-600" required>
                    ...
                    @foreach ($suppliers as $supplier)
                        <option value="{{ $supplier->supplier_id }}" {{ old('supplier_id', $purchaseOrder->supplier_id) == $supplier->supplier_id ? 'selected' : '' }}>{{ $supplier->supplier_name }}</option>
                    @endforeach
                </select>
                @error('supplier_id')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
            ...
            <div id="items" class="space-y-4">
                ...
                <div class="item grid gap-4 md:grid-cols-4 items-end">
                    <div>
                        ...
                        <select name="items[{{ $index }}][product_id]" class="mt-1 block w-full rounded-xl border-slate-300 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100 focus:ring-brand-600 focus:border-brand-600" required>
                            ...
                            @foreach ($products as $product)
                                <option value="{{ $product->product_id }}" {{ $item->product_id == $product->product_id ? 'selected' : '' }}>{{ $product->product_name }} (Giá bán: {{ number_format($product->price, 0, ',', '.') }} VNĐ)</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" name="items[{{ $index }}][quantity]" value="{{ old('items.' . $index . '.quantity', $item->quantity) }}" min="1" class="mt-1 block w-full rounded-xl border-slate-300 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100 focus:ring-brand-600 focus:border-brand-600" required>
                    ...
                    <div>
                        ...
                        <input type="number" name="items[{{ $index }}][price]" value="{{ old('items.' . $index . '.price', $item->price) }}" min="0" step="0.01" class="mt-1 block w-full rounded-xl border-slate-300 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100 focus:ring-brand-600 focus:border-brand-600" required>
